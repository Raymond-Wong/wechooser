...
from customize.models import Subscribe_Record
...
def update_statistic(params, diff=1, aid=None):
  ...
  if record.count() <= 0:
    total_amount = User.objects.filter(source_type=0).count()
    record = [Subscribe_Record(date=today, record_type=0, total_amount=total_amount)]
  record = record[0]
  if diff > 0:
